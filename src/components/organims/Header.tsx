...
const Header: React.FC<HeaderProps> = ({ page }) => {
	return (
		<div className='h-20 shadow-sm sticky top-0 bg-white/70 w-full flex items-center justify-between gap-16 z-20 px-8 py-4'>
			...
			<div className='flex-1 flex items-center gap-28'>
				<ul className='flex-1 flex gap-16 text-sm items-center justify-end'>
					<Link
						to='/'
						className={clsx(
							"p-4 w-fit h-10 flex items-center justify-center hover:bg-neutral-100 cursor-pointer rounded-full font-medium",
							{
					...
					<Link
						to='/works'
						className='p-4 w-fit h-10 flex items-center justify-center hover:bg-neutral-100 cursor-pointer rounded-full font-medium'>
						My works
					</Link>
					<Link
						to='/info'
